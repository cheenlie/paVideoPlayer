...
import static com.cheenlie.panoramavideo.ShaderUtil.createProgram;
...
//    public Triangle(MySurfaceView mv,float scale,float aHalf,int n)
//    {
...
//    }
    ...
    public Triangle(MySurfaceView mv, float scale)
    {
    	Resources myResource = mv.getResources();
    	//调用初始化顶点数据的initVertexData方法
    	initShere(myResource, scale);
    	//调用初始化着色器的intShader方法
    	initShader(mv);
    }
    ...
    public void initShere(Resources r, float scale) //大小，黄金长方形长边的一半，分段数
	{
    	String filename = "sphere.obj";
    	LoadOBJ obj = new LoadOBJ(r, filename, scale);
		//obj.loadObj(r, filename, scale);
		float[] vertex=obj.vertices;
		float[] normals=obj.vNormal;
		float[] textures=obj.vTexCoord;
